...
type PrivateListConfig = {
    name: string;
    fileName: string;
    description: string;
    parser: (rows: CsvRow[], listId: number) => PrivateParsedRows;
};

type CsvRow = Record<string, string>;

type EntityInput = typeof sanctionsEntities.$inferInsert;
type CryptoInput = typeof sanctionsCryptoAddresses.$inferInsert;

type PrivateParsedRows = {
    entities: EntityInput[];
    crypto: Array<Omit<CryptoInput, "entityId"> & { entityKey: string }>;
};

const PRIVATE_LISTS_DIR = process.env.PRIVATE_LISTS_DIR || "/app/private-lists";

export const PRIVATE_LISTS: PrivateListConfig[] = [
    {
...
function parseExtremistas(rows: CsvRow[], listId: number): PrivateParsedRows {
    const entities: EntityInput[] = [];
    const crypto: PrivateParsedRows["crypto"] = [];
    ...
    for (const row of rows) {
        ...
        entities.push(
            makeEntity(listId, key || mainAddress, name || mainAddress, "entity", [], [program], null, remarks || null),
        );
...
function parseJsonishArray(value: string): string[] {
    ...
    try {
        ...
        if (typeof parsed === "string") return [parsed].map(clean).filter(Boolean);
    } catch {
...
function detectCryptoNetwork(address: string): string {
    ...
    if (/^[LM3][a-km-zA-HJ-NP-Z1-9]{26,33}$/.test(address)) return "LTC";
    if (/^X[1-9A-HJ-NP-Za-km-z]{33}$/.test(address)) return "DASH";
    return "UNKNOWN";
}

function allWalletCandidates(...values: string[]): string[] {
...
function parseScams(rows: CsvRow[], listId: number): PrivateParsedRows {
    const entities: EntityInput[] = [];
    const crypto: PrivateParsedRows["crypto"] = [];
    ...
    for (const row of rows) {
        ...
        entities.push(
            makeEntity(
                ...
                key || name,
                name,
                schema === "cryptowallet" ? "entity" : schema || "entity",
                parseJsonishArray(row.aliases),
                programs,
                parseJsonishArray(row.countries)[0] ?? null,
                [
                    clean(row.addresses),
...
function parseRiskWalletList(program: string) {
    return (rows: CsvRow[], listId: number): PrivateParsedRows => {
        const entities: EntityInput[] = [];
        const crypto: PrivateParsedRows["crypto"] = [];
        ...
        for (const row of rows) {
            ...
            entities.push(
                makeEntity(
                    ...
                    name,
                    "entity",
                    [clean(row.Classification), clean(row.Wallet_Name)].filter(Boolean),
                    programs,
                    clean(row.Country) || null,
                    [clean(row.Info), clean(row.Balance), clean(row.RiskPor)]
                        .filter(Boolean)
                        .join(" | ") || null,
                ),
            );
...
export async function syncPrivateCsvList(config: PrivateListConfig, listId: number): Promise<SyncCounts> {
    const filePath = path.join(PRIVATE_LISTS_DIR, config.fileName);

    if (!existsSync(filePath)) {
        throw new Error(`Private CSV not found: ${filePath}`);
    }

    const rows = readCsvRows(await readFile(filePath, "utf8"));
    ...
    let entityCount = 0;
    let addressCount = 0;
    const entityKeyToId: Record<string, number> = {};
    const batchSize = 200;
    ...
    for (let i = 0; i < parsed.entities.length; i += batchSize) {
        ...
        for (const entity of inserted) {
            if (entity.sdnId) entityKeyToId[entity.sdnId] = entity.id;
        }
    }
...
export function privateListSource(config: PrivateListConfig): string {
